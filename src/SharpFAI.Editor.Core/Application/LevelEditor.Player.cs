...
/// <summary>
/// LevelEditor - 播放功能部分
/// 从 EditorPlayer.Playback.cs 迁移
/// </summary>
public partial class LevelEditor : IPlayer
{
    ...
    public void UpdatePlayer(double delta)
    {
        ...
        // 播放控制
        // Playback controls
        if (GraphicsContext.KeyboardState.IsKeyPressed(Keys.Space))
        {
            if (!_isPlaying)
            {
                StartPlay();
        ...
        if (GraphicsContext.KeyboardState.IsKeyPressed(Keys.R))
        {
            ResetPlayer();
            StartPlay();
        }
        ...
        if (GraphicsContext.KeyboardState.IsKeyPressed(Keys.P) && _initialized)
        {
            if (_isPlaying)
            {
                PausePlay();
