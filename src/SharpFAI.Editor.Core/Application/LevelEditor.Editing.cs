...
public partial class LevelEditor
{
    ...
    private void HandleInput()
    {
        ...
        var currentMousePos = new Vector2(GraphicsContext.MouseState.X, GraphicsContext.MouseState.Y);
    ...
    {
        // 左键或中键或右键按下时拖动摄像机
        bool isAnyButtonDown = GraphicsContext.MouseState.IsButtonDown(MouseButton.Left) ||
                               GraphicsContext.MouseState.IsButtonDown(MouseButton.Middle) ||
                               GraphicsContext.MouseState.IsButtonDown(MouseButton.Right);
    ...
    {
        if (_lastScrollDelta.Y != 0)
        {
            // 计算缩放因子 - 与 EditorPlayer 保持一致
            // ScrollDelta.Y < 0（向下滚动）→ 1.25f（放大）
            // ScrollDelta.Y > 0（向上滚动）→ 0.8f（缩小）
            float zoomFactor = _lastScrollDelta.Y < 0 ? 1.25f : 0.8f;

            // 获取鼠标在屏幕上的位置
            Vector2 mouseScreenPos = new Vector2(GraphicsContext.MouseState.X, GraphicsContext.MouseState.Y);
